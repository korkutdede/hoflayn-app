...
import { createCatalog, listCatalogs } from "@/lib/services/catalogs";

export async function GET(request: Request) {
  try {
    const tenant = await requireApiOnboardedTenant(request);
    return apiSuccess(await listCatalogs(tenant));
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    if (tenant.role === "member") {
      return apiFailure(new ApiError(403, "forbidden", "catalogs.error.role"));
    }
    return apiSuccess(
      await createCatalog(tenant, await readJson(request)),
      201,
    );
  } catch (error) {
